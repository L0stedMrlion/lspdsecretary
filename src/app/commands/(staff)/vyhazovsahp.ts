...
import { SAHP_LOGO_URL } from "../../../config/constants";
...
export const chatInput = async ({
  ...
  const embedContent = `# 🤠 SAHP - Ukončení pracovního poměru
...
tímto Vám oficiálně oznamujeme **ukončení Vašeho pracovního poměru** u San Andreas Highway Patrol s okamžitou platností.
      ...
      try {
        ...
        if (targetMember) {
          if (targetMember.kickable) {
            await targetMember.kick(`SAHP Vyhazov: ${reason}`);
